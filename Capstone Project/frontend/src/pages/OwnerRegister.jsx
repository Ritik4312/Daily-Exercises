...
function OwnerRegister() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setStatus }) => {
      ...
      if (res.ok) setTimeout(() => navigate("/owner/login"), 1000);
    }
  });
  ...
  return (
    <div className="container mt-5" style={{ maxWidth: "450px" }}>
      <div className="card shadow-sm p-4">
        ...
        <button className="btn btn-success w-100" onClick={formik.handleSubmit}>Register</button>
        <p className="text-center mt-3 mb-0">Already have account? <Link to="/owner/login">Login</Link></p>
      </div>
    </div>
  );
}
